...
class Face_Detection:
    def __init__(self):
        ...
        self.img_subscriber = rospy.Subscriber('/webcam', Image,self.img_callback)
        self.results_publisher = rospy.Publisher('/cv/face/personIs', String)
        self.db_base_path = rospkg.RosPack().get_path('cv')
        self.TIMEOUT = 60
        self.img_pub = rospy.Publisher('/face/img', Image, queue_size=1,latch=True)
    ...
    def jason_callback(self, msg):        
        ...
        self.img_pub.publish(self.bridge.cv2_to_imgmsg(self.cv_image,encoding='passthrough'))
